...
class FileVLM:
    ...
    def adjudicate(self, item, image_path) -> dict:
        ...
        if choice == "custom" and item.kind != "page_completeness" and not v.get("crop_bbox") and (not isinstance(corrected, str) or not corrected.strip()):
        ...
        return {"choice": choice, "corrected_latex": corrected,
                ...
                "model": "agent", "edits": v.get("edits", []), "inserts": v.get("inserts", []), "metadata": v.get("metadata", {}), "crop_bbox": v.get("crop_bbox"), "input_hash": item.input_hash}
