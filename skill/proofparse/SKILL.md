...
首次完整运行按 `references/workflow.md` 的固定步骤执行，避免自行拼装流程。
默认单代理执行；小批读取、按篇/阶段集中导入，局部先修、整页最后查。不得自行启动多代理全量复核或无限重查。
若本机已有用户确认的环境，直接复用。CLI 返回成功不等于视觉复查完成。
...
## 最终交付：仅 Markdown 和图片目录

解析、复查结束后，将最终资料导出到单独目录：

```text
<PY> -m proofparse.export <单篇或批量工作目录> -o <交付目录>
```

每篇只交付 `<论文名>.md` 和 `images/`。Markdown 使用 `![原图](images/...)` 相对路径嵌入图片，图注与表注保留；两者一起复制即可使用。
“图片包”默认是可直接被 Markdown 引用的文件夹；只有用户要求压缩时才额外处理压缩，不让 Markdown 指向 ZIP 内部。
不要将 document.json、qc.json、原 PDF、日志或工作清单放进交付目录。这些留在工作目录用于续跑和验证，不能提前删除导致复查中断。
交付导出不代表复查通过；尚未确认的内容如实说明，表格可用原图保留。非空交付目录不自动覆盖。
